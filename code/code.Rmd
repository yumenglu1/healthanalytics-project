...
# Run logistic regression using Smoking_Intensity (with detailed categories)
logit_model <- glm(
...
# Create a mapping of raw coefficient names to natural language labels.
coef_map <- c(
  `(Intercept)` = "Intercept",
  "Smoking_IntensityCurrent Smoker (Light)" = "Current Smoker (Light)",
  "Smoking_IntensityCurrent Smoker (Moderate)" = "Current Smoker (Moderate)",
  "Smoking_IntensityCurrent Smoker (Heavy)" = "Current Smoker (Heavy)",
  "Smoking_IntensityFormer Smoker" = "Former Smoker",
  "Smoking_IntensityNever Smoked" = "Never Smoked",
  "Smoking_IntensityUnknown" = "Unknown"
)

# Create a mapping of goodness-of-fit statistics with natural language labels.
gof_map <- tribble(
  ~raw,         ~clean,                             ~fmt,    ~dec, ~drop,
  "AIC",        "Akaike Information Criterion",     "%.2f", 2,    FALSE,
  "BIC",        "Bayesian Information Criterion",   "%.2f", 2,    FALSE,
  "Log.Lik.",   "Log-Likelihood",                   "%.2f", 2,    FALSE,
  "N",          "Observations",                     "%.0f", 0,    FALSE
)

# Generate a single standardized table with robust standard errors,
# exponentiated coefficients (odds ratios), and natural language labels.
model_table <- modelsummary(
  ...
  output = "kableExtra",
  title = "Logistic Regression Results with Robust Standard Errors"
)

# Apply kableExtra styling to match your other tables:
model_table <- kable_styling(
  model_table, 
